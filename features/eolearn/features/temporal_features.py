    ...
    def execute(self, eopatch):
        ...
        for ih, iw in it.product(range(h), range(w)):
            ...
            if self.ndvi_feature_name:
                data_pos_tr[ih, iw] = \
                    np.any(eopatch.data[self.ndvi_feature_name][:start+1, ih, iw, 0] <= self.ndvi_barren_soil_cutoff)
                data_neg_tr[ih, iw] = \
                    np.any(eopatch.data[self.ndvi_feature_name][end:, ih, iw, 0] <= self.ndvi_barren_soil_cutoff)
